...
let authenticate ~peer_uid ~peer_gid ~shared_group ~admin_group =
  try
    ...
    if not in_shared_group then
      ...
      let admin_group_entry = Unix.getgrnam admin_group in
      let permission =
        if peer_gid = admin_group_entry.Unix.gr_gid
           || user_in_group ~username ~user_entry admin_group_entry
        then
          Admin
        else
          Interactive
...
let authenticate_socket socket ~shared_group ~admin_group =
  try
    let peer_uid, peer_gid = get_peer_credentials socket in
    authenticate ~peer_uid ~peer_gid ~shared_group ~admin_group
  with
